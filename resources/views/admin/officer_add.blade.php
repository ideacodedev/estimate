@extends('layout.app')
@section('content')
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">หน้าหลัก / ข้อมูลเจ้าหน้าที่ /</span>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('add_officer') }}" method="post">
                @csrf
                <h5 class="modal-title" id="exampleModalLabel3">เพิ่มข้อมูลเจ้าหน้าที่</h5>
                <hr>
                <div class="col mb-3">
                ...
                <div class="col mb-3">
                    ...
                    <input type="email" class="form-control" required name="officer_email"
                ...
                <div class="col mb-3">
                    ...
                    <select name="room_id[]" required class="form-control select2" multiple data-placeholder="เลือก::ห้อง">
                        @foreach ($room as $for)
                            <option value="{{ $for->room_id }}">{{ $for->room_name }} (ชั้น {{ $for->room_floor }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="col mb-3">
                    <label class="form-label">Username</label>
                    <input type="text" class="form-control" required name="username" placeholder="กรุณากรอก::Username" />
                </div>
                <div class="col mb-3">
